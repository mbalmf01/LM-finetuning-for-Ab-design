...
def regression_simple(input_dim=1536):
    model = Sequential()
    #input Layer
...
def regression_nn(input_dim=1536):
    model = Sequential()
    #input Layer
...
def objective(trial, X_train, X_val, y_train, y_val):
    ...
    model = regression_model_to_opt(X_train.shape[1], hidden_units, dropout_rate)
    history = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=20, batch_size=512, verbose=0)
    val_loss = history.history["val_loss"][-1]
    return val_loss
    
def ablang_opt(input_dim):
